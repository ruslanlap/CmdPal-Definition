...
internal class ChineseDictionaryProvider : IDictionaryProvider
{
    ...
    public ChineseDictionaryProvider(HttpClient _)
    {
        // HttpClient is not needed but kept for interface compatibility
    }
    ...
    private static Dictionary<string, List<DictionaryEntry>> LoadDictionary()
    {
        ...
        var resourceName = "DefinitionExtension.Resources.cedict.txt.gz";
        ...
        try
        {
            ...
            if (stream == null)
            {
                Debug.WriteLine($"[ChineseProvider] Resource not found: {resourceName}");
                return data;
            }
        ...
        {
            Debug.WriteLine($"[ChineseProvider] Error loading dictionary: {ex.Message}");
        }
    ...
    private static DictionaryEntry? ParseLine(string line)
    {
        ...
        entry.Phonetics.Add(new PhoneticInfo { Text = pinyin });
